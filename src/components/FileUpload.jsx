...
function DropZone({ label, accept, multiple, onChange, files, color }) {
  ...
  return (
    ...
    >
      ...
      <p className="text-xs text-gray-400 mt-1">클릭 또는 드래그&드롭 (.xlsx, .xls)</p>
      {files && files.length > 0 && (
        <div className="mt-3 space-y-1">
          {files.map((f, i) => (
            <div key={i} className="text-xs bg-gray-100 rounded-lg px-3 py-1 text-gray-600 truncate max-w-xs mx-auto">
              ✅ {f.name}
            </div>
          ))}
        </div>
  ...
  return (
    <div className="max-w-3xl mx-auto py-10 px-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div>
          <p className="text-xs font-semibold text-blue-600 mb-1 uppercase tracking-wide">1. 기본매출 데이터 <span className="text-red-500">*필수</span></p>
          <DropZone
            label="기본매출 파일 업로드"
            accept=".xlsx,.xls"
            multiple={false}
            files={salesFiles}
            onChange={setSalesFiles}
            color="blue"
          />
        </div>
        <div>
          <p className="text-xs font-semibold text-green-600 mb-1 uppercase tracking-wide">2. 쿠폰이벤트 데이터 <span className="text-gray-400">(선택)</span></p>
          <DropZone
            label="쿠폰이벤트 파일 업로드 (복수 가능)"
            accept=".xlsx,.xls"
            multiple={true}
            files={couponFiles}
            onChange={setCouponFiles}
            color="green"
          />
        </div>
        <div>
          <p className="text-xs font-semibold text-purple-600 mb-1 uppercase tracking-wide">3. 광고 데이터 <span className="text-gray-400">(선택)</span></p>
          <DropZone
            label="광고 파일 업로드 (복수 가능)"
            accept=".xlsx,.xls"
            multiple={true}
            files={adFiles}
            onChange={setAdFiles}
