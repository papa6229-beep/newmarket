    ...
    <div className="min-h-screen bg-white flex flex-col">
      ...
      <nav className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-warm-100">
        <div className="max-w-6xl mx-auto px-6 h-14 flex items-center justify-between">
          ...
          <button onClick={() => navigate('/login')} className="btn-primary text-xs px-5 py-2.5 rounded-xl">
            무료로 시작하기
          </button>
        </div>
      </nav>
      ...
      <section className="flex flex-col items-center justify-center text-center px-6 pt-24 pb-20 bg-gradient-to-b from-warm-50 to-white">
        ...
        <div className="flex items-center gap-3 mb-20">
          <button onClick={() => navigate('/login')} className="btn-primary px-8 py-3.5 text-sm rounded-2xl shadow-sm">
            지금 무료로 시작하기
          ...
          <div className="flex items-end gap-1.5 h-28 mb-3">
            {BARS.map((h, i) => (
              <div key={i} className="flex-1 flex flex-col justify-end">
                {i === 9 && (
                  <div className="text-[8px] text-accent-500 font-bold text-center mb-1">▲ 최고</div>
                )}
                <div
                  className={`w-full rounded-t transition-all ${
                    i === 9 ? 'bg-accent-500' : i >= BARS.length - 2 ? 'bg-accent-200' : 'bg-warm-200'
                  }`}
                  style={{ height: `${h}%` }}
                />
              </div>
      ...
      <section className="py-24 px-6 bg-warm-50">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-14">
            ...
            <h2 className="text-3xl font-bold text-warm-900">쇼핑몰 데이터 분석의 모든 것</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {[
              { emoji: '📊', title: '매출 KPI 분석', badge: '자동화', desc: '총 매출, 전환율, 객단가, 방문자 수 등 핵심 지표를 자동 계산하고 전년 대비 성장률을 시각화합니다.' },
              { emoji: '🎯', title: '이벤트 효율 비교', badge: '핵심', desc: '쿠폰 이벤트별 매출 기여도, 신규/기존 회원 비율, ROI를 한눈에 비교하여 최적 전략을 도출합니다.' },
              { emoji: '📢', title: '광고 ROAS 분석', badge: 'AI 기반', desc: '채널별 광고 비용 대비 수익(ROAS)을 분석하고, 예산 재배분을 위한 구체적 가이드를 제공합니다.' },
            ].map((f, i) => (
              <div key={i} className="card p-7 hover:shadow-md transition-all hover:-translate-y-0.5">
                <div className="w-12 h-12 rounded-2xl bg-accent-50 border border-accent-100 flex items-center justify-center text-2xl mb-5">{f.emoji}</div>
      ...
      <section className="py-24 px-6">
        <div className="max-w-lg mx-auto text-center">
          ...
          <p className="text-warm-400 text-sm mb-10 leading-relaxed">
            복잡한 세팅 없이 매출 데이터 엑셀 파일을 업로드하는 것만으로<br />AI가 즉시 분석을 시작합니다.
          </p>
          <div className="card p-8 flex flex-col items-center gap-4">
            ...
            <button onClick={() => navigate('/login')} className="btn-primary w-full justify-center py-3 rounded-xl">
              분석 시작하기
            </button>
          </div>
        </div>
      ...
      <footer className="border-t border-warm-100 py-10 px-6">
        <div className="max-w-5xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            <div className="w-6 h-6 rounded-lg bg-accent-500 flex items-center justify-center shrink-0">
